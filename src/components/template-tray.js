...
class TemplateTray extends Component {
  ...
  render(){
    ...
    const templatesList = this.props.templates.map((template)=>{
      return <li key={template.id} onClick={(e) => this.handleClick(template.body, e)} className="btn-dark btn-block">{template.label}  <button onClick={(e) => this.openEditForm(template.id, template.label, template.body)} >Edit</button></li>;
    });
